...
/* Mod by YZhao (common/schedule.c)
 * Date: 16/9/2015
 *
 ...
 * 1st: 9/23/2015
 * 2nd: 9/29/2015
 * add spinlock irq like that in function vcpu_runstate_get(...)
 *

void vcpu_ctxt_get(struct vcpu* v, struct cpu_user_regs* ctxt) {
    spinlock_t* lock = likely(v == current) ? NULL : vcpu_schedule_lock_irq(v);
    
    memcpy(ctxt, &v->ctxt, sizeof(*ctxt));
...
void vcpu_wake(struct vcpu *v)
{
    ...
    if ( likely(vcpu_runnable(v)) )
    {
         ...
         */
        if (v->processor == 0)
        TRACE_5D(TRC_SCHED_S_GET,
         v->domain->domain_id, v->vcpu_id, v->domain->domain_id, 
         v->vcpu_id, 777777);
        /* End of Mod */
...
ret_t do_sched_op(int cmd, XEN_GUEST_HANDLE_PARAM(void) arg)
{
     ...
     *
     */
    case SCHEDOP_smp_block:
    {

        struct vcpu* v = current;
        int cpu = smp_processor_id();
        if (cpu == 0)
            TRACE_5D(TRC_SCHED_OVERHEAD,
                 v->domain->domain_id, v->vcpu_id, 333333, 
                 333333, NOW());
...
static void schedule(void)
{
    ...
    struct scheduler* dsched = NULL;
    struct vcpu* dnext_vcpu = NULL;
    ...
    /* Mod by YZhao
     * 16/12/2015
     * send a virq to a target vcpu once these conditions are met.
     * function next_vcpu() is defined in sched_credit.c.
     */
    dsched = this_cpu(scheduler);
    dnext_vcpu = dsched->next_vcpu_top(dsched, prev);
    /*
        if ((prev != dnext_vcpu) && vcpu_runnable(prev) && (prev->domain->domain_id == 1) 
                  && (prev->is_delayed == 0)) {

            if (cpu == 0)
                TRACE_5D(TRC_SCHED_OVERHEAD,
                     prev->domain->domain_id, prev->vcpu_id, dnext_vcpu->domain->domain_id, 
                     dnext_vcpu->vcpu_id, NOW());

            send_guest_vcpu_virq(prev, VIRQ_UPCALL);

            set_timer(&sd->s_timer, now + MILLISECS(1));
            prev->is_delayed = 1;

            pcpu_schedule_unlock_irq(lock, cpu);
            trace_continue_running(prev);
            return continue_running(prev);
        }
        */
    if (cpu == 0)
    TRACE_5D(TRC_SCHED_CHECK_RUNSTATE,
         prev->domain->domain_id, prev->vcpu_id, 333333, 
         333333, vcpu_runnable(prev));
     ...
     *
    if (cpu == 0)
    TRACE_5D(TRC_SCHED_S_TIMER,
         prev->domain->domain_id, prev->vcpu_id, next->domain->domain_id, 
         next->vcpu_id, NOW());
    ...
    **/
    ...
    /* Mod by YZhao
     * 16/12/2015
     * reset the is_delayed's value so that this vcpu can send
     * a virq again.
     */

    /*
    if ((cpu == 0) && (prev->is_delayed == 1)) {
        dnext_vcpu = dsched->next_vcpu_bottom(dsched, cpu);
    }
    */
    ...
    TRACE_6D(TRC_SCHED_SWITCH,
             ...
             prev->ctxt_counter, prev->arch.user_regs.eip);
    ...
    /* Mod by YZhao
     * 1st: 18/11/2015
     * send a interrupt to a runnable vcpu.
     *
    if (vcpu_runnable(prev) && (prev->domain->domain_id != 0) 
            && (prev->vcpu_id == 1)) {
        send_guest_vcpu_virq(prev, VIRQ_UPCALL);
        prev->intr_counter += 1;
    }
...
void context_saved(struct vcpu *prev)
{
    ...
    if ( unlikely(test_bit(_VPF_migrating, &prev->pause_flags)) ) {
        ...
        if (prev->processor == 0)
        TRACE_5D(TRC_SCHED_S_GET,
         prev->domain->domain_id, prev->vcpu_id, prev->domain->domain_id, 
         prev->vcpu_id, 999);
        **/
...
static void s_timer_fn(void *unused)
{
    ...
    TRACE_4D(TRC_SCHED_S_TIMER,
             current->domain->domain_id, current->vcpu_id, 0, NOW());

    if ((current != next) && vcpu_runnable(current) && (current->domain->domain_id != 0) 
            && (current->vcpu_id == 1 || current->vcpu_id == 2)) {

        TRACE_4D(TRC_SCHED_S_TIMER,
             current->domain->domain_id, current->vcpu_id, 1, NOW());

        send_guest_vcpu_virq(current, VIRQ_UPCALL);
        //stop_timer(&sd->s_timer);

        TRACE_4D(TRC_SCHED_S_TIMER,
             current->domain->domain_id, current->vcpu_id, 2, NOW());

        //set_timer(&sd->s_timer, NOW() + MILLISECS(1));

        TRACE_4D(TRC_SCHED_S_TIMER,
             current->domain->domain_id, current->vcpu_id, 3, NOW());
    }
