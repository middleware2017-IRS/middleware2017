...
long do_vcpu_op(int cmd, int vcpuid, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    case VCPUOP_get_runstate_info:
    {
        ...
        struct schedule_data* sd = NULL;
        sd = &per_cpu(schedule_data, cpu);
        for_each_online_cpu(cpu) {
           if (cpu == 0) {
               struct scheduler *sched;
               sched = per_cpu(scheduler, cpu);
               next = sched->next_vcpu_bottom(sched, cpu);
            } 
        }

        if (v->processor == 0)
        TRACE_5D(TRC_SCHED_S_GET,
         v->domain->domain_id, v->vcpu_id, next->domain->domain_id, 
         next->vcpu_id, sd->curr->domain->domain_id);
        * End od Mod */

        vcpu_runstate_get(v, &runstate);

 
        //runstate.curr_domain = sd->curr->domain->domain_id;    
        //runstate.next_domain = next->domain->domain_id;
     ...
     *
     */
    case VCPUOP_get_cpu_user_regs_info:
    {
        struct cpu_user_regs regs;

        memcpy(&regs, &v->arch.user_regs, sizeof(regs));
    ...
    case VCPUOP_register_runstate_memory_area:
    {
        ...
        * End of Mod */
        ...
        if ( !guest_handle_okay(area.ctxt.r, 1) ) {
            printk("Guest Handle is not ok!\n");
        }
        ...
        * End of Mod */
        ...
        if ( v == current )
        {
            ...
            if ((v->domain->domain_id != 32767) && (v->domain->domain_id != 0)) {
                 __copy_to_guest(ctxt_guest(v), &v->ctxt, 1);
                 
                 
                 printk("Before Copy_to_Guest Domain: %d, VCPU: %d\n", v->domain->domain_id,
                             v->vcpu_id);
                 printk("After Copy_to_Guest Domain: %d, VCPU: %d\n", v->domain->domain_id,
                             v->vcpu_id);
#ifdef CONFIG_COMPAT
                 printk("Define COMPAT! %lu\n", NOW());
                 if (area.ctxt.p == NULL) {
                     printk("area.ctxt.p is NULL at %lu\n", NOW());
                 }
                 if (area.ctxt.p != NULL ) {
                     printk("area.ctxt.p is not NULL at %lu\n", NOW());
                 }
                 if (area.addr.v == NULL) {
                     printk("area.addr.v is NULL at %lu\n", NOW());
                 }
                 if (area.addr.v != NULL ) {
                     printk("area.addr.v is not NULL at %lu\n", NOW());
                 }
#endif
            * End of Mod */
        ...
        else
        {
            ...
            if ((v->domain->domain_id != 32767) && (v->domain->domain_id != 0)) {
                vcpu_ctxt_get(v, &user_regs);
                //printk("In Else Branch Copy_to_Guest Domain: %d, VCPU: %d\n", v->domain->domain_id,
                //          v->vcpu_id);
                __copy_to_guest(ctxt_guest(v), &user_regs, 1);
            }
