...
__runq_tickle(unsigned int cpu, struct csched_vcpu *new)
{
     ...
     * 1st: 6/1/2016
     *
    if (cur->vcpu->processor == 0) {
    TRACE_5D(TRC_SCHED_S_GET,
     cur->vcpu->domain->domain_id, cur->vcpu->vcpu_id, cur->vcpu->domain->domain_id, 
     cur->vcpu->vcpu_id, cur->pri);
    TRACE_5D(TRC_SCHED_S_GET,
     new->vcpu->domain->domain_id, new->vcpu->vcpu_id, new->vcpu->domain->domain_id, 
     new->vcpu->vcpu_id, new->pri);
    ...
    **/
    ...
    if ( !cpumask_empty(&mask) )
    {
         ...
         * 1st: 6/1/2016
         *
        if (new->vcpu->processor == 0)
        TRACE_5D(TRC_SCHED_S_GET,
         new->vcpu->domain->domain_id, new->vcpu->vcpu_id, new->vcpu->domain->domain_id, 
         new->vcpu->vcpu_id, 111111);
        **/
    ...
    /* 
        ...
        if (new->vcpu->domain->domain_id == 1 && new->vcpu->vcpu_id == 0) {

            cpumask_set_cpu(new->vcpu->processor, &mask);
            cpumask_raise_softirq(&mask, SCHEDULE_SOFTIRQ);
        }
    }
    */
...
csched_vcpu_wake(const struct scheduler *ops, struct vcpu *vc)
{
         ...
         !test_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags) )
    {
         ...
         * 1st: 6/1/2016
         */
        if (vc->domain->domain_id != 0 && (vc->vcpu_id == 0 
                    || vc->vcpu_id == 1)) {
            //svc->pri = CSCHED_PRI_TS_UNDER;
            //vc->real_block = 1;
        }
        if (vc->processor == 0)
        TRACE_5D(TRC_SCHED_S_GET,
         vc->domain->domain_id, vc->vcpu_id, vc->domain->domain_id, 
         vc->vcpu_id, 222222);
        /**/
        ...
        list_for_each_safe( iter_vcpu, next_vcpu, &sdom->active_vcpu )
        {
            ...
            else
            {
                ...
                if (svc->vcpu->domain->domain_id == 1) {
                    send_guest_vcpu_virq(svc->vcpu, VIRQ_UPCALL);
                }
                **/
 ...
 * 1st: 16/12/2015
 * 2nd: 5/1/2016
 * This function will return the vcpu at the head of runq.
 */

static struct vcpu* csched_next_vcpu_bottom(const struct scheduler* ops, unsigned int cpu) {
    ...
    if (cpu == 0) {
        TRACE_5D(TRC_SCHED_S_GET,
        current->domain->domain_id, current->vcpu_id, next->domain->domain_id, 
        next->vcpu_id, 555555);
    }
...
static struct vcpu* csched_next_vcpu_top(const struct scheduler* ops, struct vcpu* v) {
...
const struct scheduler sched_credit_def = {
    ...
    /* Mod by YZhao
     * 14/12/2015
     * get next vcpu residing in runq.
     */

    .next_vcpu_bottom      = csched_next_vcpu_bottom,
    .next_vcpu_top      = csched_next_vcpu_top,
